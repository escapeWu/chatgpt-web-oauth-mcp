...
| `taskboard_create` / `taskboard_add_task` / `taskboard_delegate` | Persist user-decomposed subtask boards, including the original request, and submit pending items |
| `taskboard_status` / `taskboard_wait` / `taskboard_get_task` | Inspect compact board state, wait on `change` / `any_done` / `all_done`, and fetch detailed subtask metadata |
| `taskboard_collect_results` / `taskboard_cancel` / `taskboard_list` / `taskboard_purge` | Collect worktree metadata, cancel, list, and clean board metadata |
